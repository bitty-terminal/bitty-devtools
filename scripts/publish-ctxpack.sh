...
# bitty-devtools checkout (the trigger is the merge closeout, NOT a git hook:
...
# bitty-terminal/bitty-devtools-workflow.
#
# What it does:
#   1. Exports `carryctx export --pack-format dir` from this repo into a
#      staging clone of bitty-terminal/bitty-devtools-workflow as
#      <UTC-date>-<devtools-main-sha>/.
...
#                  <repo>/../bitty-devtools-workflow).
#   --url URL      mirror remote for a fresh clone
#                  (default: $WORKFLOW_MIRROR_URL or
#                  https://github.com/bitty-terminal/bitty-devtools-workflow.git).
...
SOURCE_REPO="${WORKFLOW_SOURCE_REPO:-bitty-devtools}"
